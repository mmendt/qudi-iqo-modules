...
class FollowLogic(LogicBase):
    """ FIXME
    """

    _min_step_interval = ConfigOption(name='min_step_interval', default=0.1, missing='warn')
    _sim_hardware_delay = ConfigOption(name='sim_hardware_delay', default=0, missing='warn')

    _follow_velocity = StatusVar(name='follow_velocity', default=0.0002)

    sigPositionChanged = QtCore.Signal(tuple)
    ...
    def __follow_loop_body(self):
        with self._thread_lock:
            ...
            if np.array_equal(self._target_pos, self._current_pos):
                self.__last_follow = None
                print('====> loop finished!')
                return
            ...
            connecting_vec = self._target_pos - self._current_pos
            ...
            if max_step_distance < distance_to_target:
                ...
                self._current_pos += max_step_distance * direction_vec
            else:
                self._current_pos[:] = self._target_pos[:]
            sleep(self._sim_hardware_delay)
            self.sigPositionChanged.emit(tuple(self._current_pos))
